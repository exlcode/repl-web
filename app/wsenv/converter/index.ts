...
import { addFileProtocol, addPathPrefix } from 'utils/files'
import IMarkdownString = monaco.IMarkdownString
...
export class MonacoToProtocolConverter {
  ...
  asRange(
    range: Partial<monaco.IRange> | undefined | null
  ...
  asTextDocumentIdentifier(
    ...
    return {
      uri: addFileProtocol(addPathPrefix(model.uri.toString(), envKey))
    }
  }
  ...
  asTextDocumentPositionParams(
    ...
    return {
      textDocument: this.asTextDocumentIdentifier(model, envKey),
  ...
  asCompletionItem(item: monaco.languages.CompletionItem): CompletionItem {
    ...
    if (is.number(item.kind)) {
      if (
        monaco.languages.CompletionItemKind.Text <= item.kind &&
        item.kind <= monaco.languages.CompletionItemKind.Reference
      ) {
...
export class ProtocolToMonacoConverter {
  asResourceEdits(
    resource: monaco.Uri,
    edits: TextEdit[]
  ): monaco.languages.IResourceEdit[] {
  ...
  asCodeLens(
    item: CodeLens | undefined | null
  ): monaco.languages.ICodeLensSymbol | undefined
  asCodeLens(
    item: CodeLens | undefined | null
  ): monaco.languages.ICodeLensSymbol | undefined {
    if (!item) {
  ...
  asDocumentHighlight(
    ...
    const range = this.asRange(item.range)!
    const kind = is.number(item.kind)
  ...
  asDefinitionResult(
    item: Definition | undefined | null
  ...
  asCompletionResult(
    result: CompletionItem[] | CompletionList | undefined
  ):
  ...
  asCompletionInsertText(
    ...
    if (item.textEdit) {
      ...
      const text =
        item.insertTextFormat === InsertTextFormat.Snippet ? { value } : value
      return {
        text,
        range,
        fromEdit: true
  ...
  asRange(range: RecursivePartial<Range> | null):
    | monaco.Range
    | Partial<monaco.IRange>
    | null
  asRange(
  ...
  asPosition(position: Partial<Position> | null):
    | monaco.Position
    | Partial<monaco.IPosition>
    | null
  asPosition(
...
export const p2m = new ProtocolToMonacoConverter()
export const m2p = new MonacoToProtocolConverter()
